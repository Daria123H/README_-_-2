...
void print_stack() {
    uintptr_t sp;
    asm volatile ("movq %%rsp, %0" : "=r"(sp));  // Отримуємо поточний адрес стеку
    printf("Stack pointer (RSP): %p\n", (void*)sp);
}

void function_c() {
    printf("Inside function_c\n");
    print_stack();  // Подивимося, де знаходиться стек
}
...
break function_a
run
bt    # Вивід стеку викликів
info registers
si    # Крокуємо по інструкціях
Вивід
